...
protected:
  SensorDevice(const char *deviceName, Print &out)
    : m_out(out),
      m_maxUpdateMsJson(m_settingsJson[0]),
      m_jsonState(m_sensorDeviceJson)
  {
    m_lastUpdateMs = 0;
    
    // Initalize device settings JSON
    m_maxUpdateMsJson = Json::Int("maxUpdateMs", SENSOR_DEVICE_DEFAULT_MAX_UPDATE_MS);

    // Create sensor device JSON object
    m_fieldsJson[0] = Json::String("name", deviceName);
    m_fieldsJson[1] = Json::Object("settings", m_settingsJson);
